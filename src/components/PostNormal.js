            ...
            <tr key={post.id}>
              ...
              <td width="60%">
                <Link style={{ all: "unset", cursor: "pointer" }} key={post.id} to={`/post/${post.id}`} postListWithPaging={postListWithPaging} txtSearch={txtSearch}
                  state={{ id: post.id, page: state.page, search: txtSearch.current?.value, postListWithPaging, seriesId: state?.seriesId, parentId: state?.seriesId, boardId: post?.boardVO?.id, likeCount:post?.likeCount, dislikeCount:post.dislikeCount }}>{/*시리즈아이디필요*/}
                  {post.title}</Link>
              </td>
